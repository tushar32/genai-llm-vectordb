...
export async function up(knex: Knex): Promise<void> {
  ...
  if (!(await knex.schema.hasTable('embeddings'))) {
    ...
    await knex.raw('CREATE INDEX IF NOT EXISTS embeddings_metadata_gin_idx ON embeddings USING gin(metadata)');
  }

  // Create users table for API key management if it doesn\'t exist
  ...
  // Create query logs for analytics if it doesn\'t exist
  if (!(await knex.schema.hasTable('query_logs'))) {
  ...
  // Create backward compatibility table if it doesn\'t exist
  if (!(await knex.schema.hasTable('langchain_pg_embedding'))) {
    ...
    await knex.raw(`
      CREATE INDEX IF NOT EXISTS langchain_pg_embedding_vector_cosine_idx 
